...
//  slot-machine
//
//  Created by Jindong Du on 11/29/23.
//
...
struct ContentView: View {
    @State public var symbols = ["eating","happy","love","scary","sleeping","lucky"]
    @State public var numbers = [0,1,2,3,4,5]
    @State public var counter = 0
    @State public var lucky = 0
    ...
    var body: some View {
        ZStack{
                ...
                VStack(spacing: 15){
                    HStack(spacing:35){
                        Hexagon().fill(Color.white.opacity(0.8))
                            ...
                            .overlay(Image(symbols[numbers[0]]).resizable().scaledToFit().frame(width: 80,height: 70,alignment: .center)
                                .cornerRadius(80)
                                .shadow(color:.gray, radius: 4, x:4,y:5))
                        
                        Hexagon().fill(Color.white.opacity(0.8))
                            .frame(width: 100,height: 120,alignment: .center)
                            .overlay(Image(symbols[numbers[1]]).resizable().scaledToFit().frame(width: 80,height: 70,alignment: .center)
                                .cornerRadius(80)
                                .shadow(color:.gray, radius: 4, x:4,y:5))
                    }
                    Hexagon().fill(Color.white.opacity(0.8))
                        .frame(width: 100,height: 120,alignment: .center)
                        .overlay(Image(symbols[numbers[2]]).resizable().scaledToFit().frame(width: 80,height: 70,alignment: .center)
                            .cornerRadius(80)
                            .shadow(color:.gray, radius: 4, x:4,y:5))
                    HStack(spacing:35){
                        Hexagon().fill(Color.white.opacity(0.8))
                            .frame(width: 100,height: 120,alignment: .center)
                            .overlay(Image(symbols[numbers[3]]).resizable().scaledToFit().frame(width: 80,height: 70,alignment: .center)
                                .cornerRadius(80)
                                .shadow(color:.gray, radius: 4, x:4,y:5))
                        
                        Hexagon().fill(Color.white.opacity(0.8))
                            .frame(width: 100,height: 120,alignment: .center)
                            .overlay(Image(symbols[numbers[4]]).resizable().scaledToFit().frame(width: 80,height: 70,alignment: .center)
                                .cornerRadius(80)
                                .shadow(color:.gray, radius: 4, x:4,y:5))
                    }
                    Button(action: {
            ...
            .alert(item: $showingAlert){
                ...
                switch alert {
                case .success:
                    return Alert(title: Text("Lucky Day!"), message: Text("Love you!"),dismissButton: .cancel())
                case .failure:
                    return Alert(title: Text("Oooopppsss"), message: Text("Still Love You!"),dismissButton: .cancel())
                }
            }
        }
    }
